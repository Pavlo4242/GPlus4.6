...
class Archive(
    ...
    @Command("safety_check", aliases = ["sc"], help = "Run safety check for detection risks")
    fun safetyCheck(args: List<String>) {
        scope.launch {
            try {
                val result = com.grindrplus.utils.StealthConfig.performSafetyCheck()

                val message = buildString {
                    if (result.isSafe) {
                        append("✓ Safety check passed\n\n")
                    } else {
                        append("⚠ Issues found:\n")
                        result.issues.forEach { append("• $it\n") }
                        append("\n")
                    }
    ...
    @Command("apply_stealth", aliases = ["stealth"], help = "Apply stealth mode settings")
    fun applyStealth(args: List<String>) {
        try {
            com.grindrplus.utils.StealthConfig.applySafetySettings()
            com.grindrplus.utils.StealthConfig.initializeStealthMode()

            GrindrPlus.showToast(
                Toast.LENGTH_LONG,
                "Stealth mode enabled. Restart app for full effect."
            )
        } catch (e: Exception) {
            GrindrPlus.showToast(Toast.LENGTH_LONG, "Error: ${e.message}")
        }
