    ...
    suspend fun archiveProfilePhotos(profileId: String): Result<List<PermanentMedia>> =
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                profilePhotos.chunked(settings.maxBatchSize).forEach { batch ->
                    batch.forEach { row ->
                        ...
                        if (photoUrl != null) {
                            try {
                                ...
                                // Add realistic delay between downloads
                                if (settings.addRandomDelay) {
                                    val delay = kotlin.random.Random.nextLong(
                                        settings.minRandomDelayMs.toLong(),
                                        settings.maxRandomDelayMs.toLong()
                                    )
                                    kotlinx.coroutines.delay(delay)
                                } else {
                                    kotlinx.coroutines.delay(settings.delayBetweenDownloads.toLong())
                                }
                            } catch (e: Exception) {
                    ...
                    if (batch != profilePhotos.last()) {
                        kotlinx.coroutines.delay(settings.delayBetweenDownloads.toLong() * 2)
                    }
                }
